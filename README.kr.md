...
Hyacine은 한국어 Discord 서버용 **자가 호스팅 커뮤니티 유틸리티 봇**입니다. 파티 모집, 서버 이벤트, 금지어 관리, GPT 대화와 이미지 생성, 게임 프로필 카드를 제공합니다. Python 3.12와 SQLite를 사용합니다. 컨테이너와 CI가 3.12로 고정되어 있어 검증된 조합은 3.12입니다 — 호스트 virtualenv를 더 높은 버전으로 만들면 `requirements.lock`이 3.13 이상에서 `audioop-lts`를 추가로 끌어옵니다. macOS·Linux 같은 **POSIX 호스트**에서 동작하며, Windows에서는 WSL2 위에서 실행합니다(아래 [실행 환경](#실행-환경) 참고).
...
- `/프로필` — 서버 가입일, 금지어 경고 횟수, 자기소개
- `/프로필설정` — 이 서버에서 보일 자기소개 (최대 200자, 비우면 삭제)
- 게임 선택 패널 — `🎮-디스코-파티`에서 게임을 고르면 해당 게임의 모집 패널 생성
- `/이벤트` — 웹 관리에서 서버별 전용 채널을 선택할 수 있고, 미지정 시 모든 채널에서 사용
- `/기본대화`, `/고급대화`, `/이미지`
- `/상태` — 이 채널의 AI 대화 상태와 오늘 남은 AI 사용 횟수
...
배포·롤백 전에 실행할 두 테스트 스위트용 host virtualenv를 먼저 설치한 뒤 이미지를 build합니다.
...
`🎮-디스코-파티` 채널에는 처음에 게임 선택 패널 하나만 표시됩니다. 게임 버튼을 누른 사용자가 방장이 되고 해당 게임의 편성 패널이 생깁니다. 역할 버튼은 참가·역할 변경에, 별도 `나가기` 버튼은 파티 이탈에 사용합니다. 마지막 인원이 나가면 해당 게임 패널이 바로 사라지고, 만든 지 24시간이 지난 패널은 10분 주기 정리 루프가 지웁니다 — 그래서 실제 삭제는 24시간에서 **최대 10분 늦을 수 있습니다**. 게임·역할 설정을 바꾼 뒤에는 봇을 재시작하세요.
...
프로덕션은 Docker Compose로 운영합니다. 백업·복구 절차는 [운영 가이드](docs/operations.kr.md)를 따르세요.
...
Docker Compose 실행, 백업, 복구, 배포, 롤백은 [운영 가이드](docs/operations.kr.md)를 참고하세요. 버전별 변경 내역은 [CHANGELOG](docs/CHANGELOG.kr.md)에 있습니다.
