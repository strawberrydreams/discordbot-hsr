...
- 캐릭터 아트와 폰트는 저장소에 넣지 않습니다. 아트는 렌더링할 때 원격에서 받아 프로세스 메모리에만 잠시 두고, 폰트는 시스템에 설치된 것을 씁니다. Docker 이미지는 `fonts-noto-cjk`(OFL)를 설치합니다. Docker 없이 운영한다면 `.env.runtime`의 `CARD_FONT_PATH`로 폰트 경로를 지정하세요.
