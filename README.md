...
- `/등록`, `/등록해제`, `/프로필카드` — 원신 · 붕괴: 스타레일 · 젠레스 존 제로 진열 캐릭터 카드
...
`/등록 <게임> <UID>`로 계정을 등록하면 `/프로필카드 <게임>`이 그 계정의 진열 캐릭터를 이름과 레벨로 보여줍니다. 데이터는 [Enka Network](https://enka.network)에서 가져옵니다.
...
- 카드는 대표 캐릭터 아트를 얹은 이미지로 나갑니다. 한국어 글리프가 있는 폰트를 찾지 못하면 같은 내용을 텍스트 임베드로 보냅니다 — 명령이 사라지지는 않습니다.
- 게임 데이터 파일은 첫 조회 때 `.enka_py/`(Compose에서는 `runtime/enka/`)로 내려받습니다. 이 디렉터리가 쓰기 가능해야 합니다.
- 캐릭터 아트와 폰트는 저장소에 넣지 않습니다. 아트는 렌더링할 때 원격에서 받아 프로세스 메모리에 최대 32 MiB만 잠시 두고, 폰트는 시스템에 설치된 것을 씁니다. Docker 이미지는 `fonts-noto-cjk`(OFL)를 설치합니다. Docker 없이 운영한다면 `.env.runtime`의 `CARD_FONT_PATH`로 폰트 경로를 지정하세요.
...
웹 관리에서는 AI 페르소나(system prompt와 인삿말), 금지어 목록, 파티 게임 목록을 편집합니다. system prompt는 최대 16,000자, 인삿말은 최대 1,974자입니다. 금지어는 저장 즉시 다시 불러오고, 페르소나는 새로 시작하는 AI 채널 세션부터, 게임 목록은 봇 재시작 후 반영됩니다. 같은 화면에서 Guild별 파티 채널과 공지 opt-in 상태를 확인하고, opt-in한 Guild에 공지를 보낼 수 있습니다.
