...
채널 지정은 환경변수가 아니라 서버별 설정입니다. 각 서버 관리자가 `/설정 파티채널`, `/설정 음악채널`, `/설정 공지허용`으로 값을 `guild_settings.db`에 저장하고 `/설정 확인`으로 볼 수 있습니다. `/모집`·`/파티`·`/나가기`·`/변경`·`/이벤트`는 어느 서버 채널에서나 사용할 수 있습니다.
