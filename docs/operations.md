...
채널 지정은 환경변수가 아니라 서버별 설정입니다. 각 서버 관리자가 `/설정 시작`으로 `🎮-디스코-파티` 채널을 만들거나 `/설정 파티채널`, `/설정 공지채널`, `/설정 공지허용`, `/설정 금지어`로 값을 `guild_settings.db`에 저장하고 `/설정 확인`으로 볼 수 있습니다. 파티 채널에는 게임 선택 패널 하나가 유지되고, 게임을 누르면 해당 게임의 활성 모집 패널이 생깁니다. 역할 버튼은 참가·변경, 별도 `나가기` 버튼은 이탈에 사용하며 마지막 인원이 나가거나 24시간이 지나면 게임 패널이 삭제됩니다. `settings/games.json`을 바꾸면 새 게임 구성과 패널을 반영하도록 봇을 재시작해야 합니다. `/이벤트`는 어느 서버 채널에서나 사용할 수 있습니다.
...
웹 관리 공지는 `/설정 공지허용`으로 opt-in한 Guild의 `/설정 공지채널`에만 보냅니다. 공지당 PNG·JPEG·GIF·WebP 이미지 1개를 최대 8 MiB까지 첨부할 수 있습니다.
...
OpenAI가 `429`와 `credit_balance_exhausted`를 반환하면 모델 혼잡이 아니라 계정 크레딧 소진입니다. OpenAI API 결제 크레딧을 충전해야 `/기본대화`와 `/고급대화`가 다시 동작합니다. `/이미지`는 별도 Gemini API를 사용하므로 영향받지 않습니다.
...
필터는 길드별로 켜고 끕니다. 기본값은 켜짐이며, `guild_settings` 스키마 v5의 `forbidden_filter_enabled`에 저장됩니다. v2~v4 DB는 기동 시 자동으로 v5가 되고 기존 서버는 켜진 상태를 유지합니다. `/설정 금지어`로 바꾼 값은 즉시 반영됩니다 — 필터가 메시지마다 DB를 조회하지 않도록 값을 프로세스에 캐시하고, 설정 명령이 그 캐시를 무효화합니다.
...
`/게임카드`는 인게임 진열창의 첫 캐릭터만 표시하며, 캐릭터 이미지는 Discord 임베드가 Enka URL에서 직접 불러옵니다.
