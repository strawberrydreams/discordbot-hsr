...
서버 간 데이터는 섞이지 않습니다. 포인트·출석·금지어 카운트·파티·설정 모두 기본키에 `guild_id`가 포함되어 스키마 수준에서 분리됩니다. 같은 사용자가 여러 서버에 있어도 잔액은 서버마다 독립입니다. DM 메시지는 귀속시킬 서버가 없어 금지어 집계에서 제외되고, 파티 참가 버튼도 서버 밖에서는 거부됩니다.
