...
캐릭터 아트와 폰트는 저장소에 커밋하지 않습니다. 아트는 렌더링 시점에 원격에서 받아 프로세스
메모리에만 두고, 폰트는 시스템에 설치된 것을 씁니다(Docker 이미지는 `fonts-noto-cjk`). 이 규칙은
`test_card_does_not_bundle_assets_in_the_repository`가 고정합니다.
